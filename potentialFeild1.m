function V = potentialFeild1(px, py, m, b, endpoints, robotpnt)
...
for line = 1:size(m, 1)
    ...
    dx = @(x)  C.*(sqrt(m(line).^2 + 1)./((sqrt((px-x).^2 + (py - m(line).*x - b(line)).^2))).^.5);
    Dx = integral(dx,start,stop);
    V0 = V0 + Dx;
end

dp =  -10./(sqrt((px - robotpnt(1)).^2 + (py - robotpnt(2)).^2));
V = V0 + dp;
end